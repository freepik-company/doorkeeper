...
IMG ?= ghcr.io/freepik-company/doorkeeper:placeholder
...
build: fmt vet check-go-target ## Build CLI binary.
	go build -o bin/doorkeeper-$(GOOS)-$(GOARCH) cmd/main.go
...
package: check-go-target ## Package binary.
	...
	@if [ "$(OS)" = "linux" ]; then \
		tar --transform="s/doorkeeper-$(GOOS)-$(GOARCH)/doorkeeper/" -cvzf dist/$(PACKAGE_NAME) -C bin doorkeeper-$(GOOS)-$(GOARCH) -C ../ LICENSE README.md; \
	elif [ "$(OS)" = "darwin" ]; then \
		tar -cvzf dist/$(PACKAGE_NAME) -s '/doorkeeper-$(GOOS)-$(GOARCH)/doorkeeper/' -C bin doorkeeper-$(GOOS)-$(GOARCH) -C ../ LICENSE README.md; \
	else \
